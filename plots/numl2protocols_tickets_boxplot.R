...
x <- binned$NumL2Protocols
y <- metrics$NumTickets


plotfile <- paste(datapath,'plots','numl2protocols_tickets_boxplot.pdf',sep="/")
pdf(plotfile, height=3, width=3)
par(mar=c(3,3.2,0.5,0), mgp=c(1.7,0.2,0))
boxplot(y~x, range=2, ylab='# of Tickets', xlab='# of L2 Protocols', xaxt='n',
...
axis(1,1:5,2:6,cex.axis=1.4,tck=0)
dev.off()
