...
x <- binned$FractionDevicesChanged
y <- metrics$NumTickets


plotfile <- paste(datapath,'plots','fracdeviceschanged_tickets_boxplot.pdf',sep="/")
pdf(plotfile, height=3, width=3)
par(mar=c(3,3.2,0.5,0), mgp=c(1.7,0.2,0))
boxplot(y~x, range=2, ylab='# of Tickets', xlab='Frac. of Devices Changed', yaxt='n', xaxt='n',
...
#axis(1,c(0.5,9.5),c(0,'O(10)'),cex.axis=1.4,tck=0)
dev.off()
