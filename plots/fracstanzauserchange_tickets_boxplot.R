...
x <- binned$FractionStanzaUserChange
y <- metrics$NumTickets


plotfile <- paste(datapath,'plots','fracstanzauserchange_tickets_boxplot.pdf',sep="/")
pdf(plotfile, height=3, width=3)
par(mar=c(3,3.2,0.5,0), mgp=c(1.7,0.2,0))
boxplot(y~x, range=2, ylab='# of Tickets', xlab='Frac. events\nw/ user change', ylim=c(0,40), yaxt='n', xaxt='n',
...
x <- binned[which(binned$Month=="2014-12"),'FractionStanzaUserChange']
y <- metrics[which(metrics$Month=="2014-12"),'NumTickets']

plotfile <- paste(datapath,'plots','fracstanzauserchange_tickets_boxplot_2014-12.pdf',sep="/")
pdf(plotfile, height=3, width=3)
par(mar=c(3,3.2,0.5,0), mgp=c(1.7,0.2,0))
boxplot(y~x, range=2, ylab='# of Tickets', xlab='Frac. events\nw/ user change', ylim=c(0,40), yaxt='n', xaxt='n',
        cex.lab=1.4, outline=FALSE)
...
x <- binned[which(binned$Month=="2014-02"),'FractionStanzaUserChange']
y <- metrics[which(metrics$Month=="2014-02"),'NumTickets']

plotfile <- paste(datapath,'plots','fracstanzauserchange_tickets_boxplot_2014-02.pdf',sep="/")
pdf(plotfile, height=3, width=3)
par(mar=c(3,3.2,0.5,0), mgp=c(1.7,0.2,0))
boxplot(y~x, range=2, ylab='# of Tickets', xlab='Frac. events\nw/ user change', ylim=c(0,40), yaxt='n', xaxt='n',
        cex.lab=1.4, outline=FALSE)
