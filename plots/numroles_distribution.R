...
counts <- table(binned$NumRoles)
counts <- counts/nrow(metrics)

plotfile <- paste(datapath,'plots','numroles_distribution.pdf',sep="/")
pdf(plotfile, height=3, width=3)
par(mar=c(3,3.5,1,0), mgp=c(2,0.4,0))
xtics <- barplot(counts, ylab='Fraction of Networks', xlab='# of Roles', xaxt='n', yaxt='n',
        cex.lab=1.4)
